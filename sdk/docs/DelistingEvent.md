...
------------ | ------------- | ------------- | -------------
**effective_date** | **datetime** | The date the security is removed from official board quotation (the trading-status change takes effect). | [optional] 
**announcement_date** | **datetime** | The date the delisting was announced. Optional — null when no separate announcement date is provided.  When populated, must be &lt;&#x3D; EffectiveDate. | [optional] 
...
delisting_event_instance = DelistingEvent(effective_date=effective_date, announcement_date=announcement_date, instrument_event_type=instrument_event_type)
