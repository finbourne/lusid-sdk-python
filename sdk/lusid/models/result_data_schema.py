...
class ResultDataSchema(BaseModel):
    ...
    def from_dict(cls, obj: dict) -> ResultDataSchema:
        ...
        _obj = ResultDataSchema.parse_obj({
            "node_value_schema": {k: None if v is None else {ik: FieldSchema.from_dict(iv) for ik, iv in v.items()} for k, v in obj.get("nodeValueSchema").items()} if obj.get("nodeValueSchema") is not None else None,
            "property_schema": {k: None if v is None else {ik: FieldSchema.from_dict(iv) for ik, iv in v.items()} for k, v in obj.get("propertySchema").items()} if obj.get("propertySchema") is not None else None,
            "address_schema": {k: None if v is None else {ik: AddressDefinition.from_dict(iv) for ik, iv in v.items()} for k, v in obj.get("addressSchema").items()} if obj.get("addressSchema") is not None else None,
        })
        return _obj
