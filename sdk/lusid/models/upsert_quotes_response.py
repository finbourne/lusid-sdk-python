    ...
    def from_dict(cls, obj: dict) -> UpsertQuotesResponse:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in UpsertQuotesResponse) in the input: " + obj)

        _obj = UpsertQuotesResponse.parse_obj({
            "href": obj.get("href"),
            "values": {k: None if v is None else {ik: Quote.from_dict(iv) for ik, iv in v.items()} for k, v in obj.get("values").items()} if obj.get("values") is not None else None,
