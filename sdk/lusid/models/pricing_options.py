...
class PricingOptions(BaseModel):
    ...
    risk_engine:  Optional[StrictStr] = Field(None,alias="riskEngine", description="Which engine computes first-order Risk/* measures. One of \"Bump\" (default: central  finite differences by bump-and-revalue - the historical behaviour, used when this is  absent), \"Parity\" (bump computes and is reported, the adjoint engine independently checks  every calculator and any disagreement fails the measure loudly - the recommended  enablement gate), or \"Adjoint\" (algorithmic differentiation where an evaluator exists,  with silent fallback to Bump elsewhere - selecting it can never reduce coverage). Available values: Bump, Adjoint, Parity.") 
    find_or_calculate:  Optional[StrictStr] = Field(None,alias="findOrCalculate", description="Whether Risk/* measures may be served from stored results instead of being recomputed.  A stored result is used only when the identity it was computed under (instrument version,  recipe, model and bump configuration, measure options, analytics version, and the content  of every market data dependency) exactly matches the identity computed fresh for this  request - a stale stored result is simply never matched. One of \"Off\" (default: compute  everything - the historical behaviour, used when this is absent), \"Parity\" (look up and  verify stored results but still compute everything, logging any divergence between stored  and computed values - the recommended enablement gate), or \"Enabled\" (serve verified  stored values and recompute only the measures with no verified stored result). Available values: Off, Parity, Enabled.") 
    ...
    def to_dict(self):
        ...
        if self.risk_engine is None and "risk_engine" in self.__fields_set__:
            _dict['riskEngine'] = None

        # set to None if find_or_calculate (nullable) is None
        # and __fields_set__ contains the field
        if self.find_or_calculate is None and "find_or_calculate" in self.__fields_set__:
            _dict['findOrCalculate'] = None
    ...
    def from_dict(cls, obj: dict) -> PricingOptions:
        ...
        _obj = PricingOptions.parse_obj({
            ...
            "risk_engine": obj.get("riskEngine"),
            "find_or_calculate": obj.get("findOrCalculate"),
            ...
            "fixed_income_valuations_using_current_face": obj.get("fixedIncomeValuationsUsingCurrentFace"),
