...
class SeriesIdentifierField(BaseModel):
    """
    A series identifier field, carrying the same fields as the CreateSeriesIdentifierField that asks for one, so  that a caller reads back what they wrote. The field category is not among them, because every field of this  shape is a series identifier; nor is a required flag, which is not the caller's to set.  # noqa: E501
    """
    ...
    @classmethod
    def from_json(cls, json_str: str) -> SeriesIdentifierField:
        """Create an instance of SeriesIdentifierField from a JSON string"""
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> SeriesIdentifierField:
        """Create an instance of SeriesIdentifierField from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return SeriesIdentifierField.parse_obj(obj)

        _obj = SeriesIdentifierField.parse_obj({
...
SeriesIdentifierField.update_forward_refs()
