...
class InterestRateSwap(LusidInstrument):
    """
    LUSID representation of an Interest Rate Swap, including:      * Vanilla (single currency fixed-float non-amortising)    * CrossCurrency (>1 currency is used by the swap legs)    * Basis (single currency, floating-floating legs of different tenors)    * Amortising (the swap has 1+ leg with amortised notional)                This instrument has multiple legs, to see how legs are used in LUSID see [How does LUSID handle instrument legs?](https://support.lusid.com/docs/how-does-lusid-handle-instrument-legs).                | Leg Index | Leg Identifier | Description |  | --------- | -------------- | ----------- |  | 1 | Pay/Receive | Cash flows representing the pay/receive leg. |  | 2 | Receive/Pay | Cash flows representing the receive/pay leg. |  | 3 | AdditionalPayments | Cash flows relating to any additional payments (optional). |  # noqa: E501
    """
    ...
    cancel_schedule: Optional[CancelSchedule] = Field(default=None, alias="cancelSchedule")
    ...
    __properties = ["instrumentType", "startDate", "maturityDate", "isNonDeliverable", "legs", "settlementCcy", "additionalPayments", "timeZoneConventions", "cancelSchedule"]
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of cancel_schedule
        if self.cancel_schedule:
            _dict['cancelSchedule'] = self.cancel_schedule.to_dict()
    ...
    def from_dict(cls, obj: dict) -> InterestRateSwap:
        ...
        _obj = InterestRateSwap.parse_obj({
            ...
            "cancel_schedule": CancelSchedule.from_dict(obj.get("cancelSchedule")) if obj.get("cancelSchedule") is not None else None
        })
