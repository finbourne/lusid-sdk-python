...
class TranslateTradeTicketRequest(BaseModel):
    ...
    def from_dict(cls, obj: dict) -> TranslateTradeTicketRequest:
        ...
        _obj = TranslateTradeTicketRequest.parse_obj({
            "tickets": {k: None if v is None else {ik: TradeTicket.from_dict(iv) for ik, iv in v.items()} for k, v in obj.get("tickets").items()} if obj.get("tickets") is not None else None,
