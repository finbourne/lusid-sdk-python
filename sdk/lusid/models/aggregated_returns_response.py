    ...
    def from_dict(cls, obj: dict) -> AggregatedReturnsResponse:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in AggregatedReturnsResponse) in the input: " + obj)

        _obj = AggregatedReturnsResponse.parse_obj({
            "href": obj.get("href"),
            "results": {k: None if v is None else {ik: List[AggregatedReturn].from_dict(iv) for ik, iv in v.items()} for k, v in obj.get("results").items()} if obj.get("results") is not None else None,
