    ...
    def from_dict(cls, obj: dict) -> BatchUpsertPortfolioTransactionsResponse:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in BatchUpsertPortfolioTransactionsResponse) in the input: " + obj)

        _obj = BatchUpsertPortfolioTransactionsResponse.parse_obj({
            "values": {k: None if v is None else {ik: Transaction.from_dict(iv) for ik, iv in v.items()} for k, v in obj.get("values").items()} if obj.get("values") is not None else None,
            ...
            "metadata": {k: None if v is None else {ik: List[ResponseMetaData].from_dict(iv) for ik, iv in v.items()} for k, v in obj.get("metadata").items()} if obj.get("metadata") is not None else None,
