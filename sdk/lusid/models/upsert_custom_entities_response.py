    ...
    def from_dict(cls, obj: dict) -> UpsertCustomEntitiesResponse:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in UpsertCustomEntitiesResponse) in the input: " + obj)

        _obj = UpsertCustomEntitiesResponse.parse_obj({
            "href": obj.get("href"),
            "values": {k: None if v is None else {ik: CustomEntityResponse.from_dict(iv) for ik, iv in v.items()} for k, v in obj.get("values").items()} if obj.get("values") is not None else None,
